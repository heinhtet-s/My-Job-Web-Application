...
export default function Footer() {
  ...
  return (
    ...
    <footer className=" text-white py-8 w-full bg-[#dceeff]">
       <div className="container mb-8">   <img src="./myjob.svg"></img></div>
            ...
            <ul className="space-y-2">
              {groupedFooter[header].map(({ id, name, link }) => (
                <li key={id} className="text-[#f69322]">
                  {link ? (
                    <Link href={`/${link}`} className="text-[#f69322]-400 hover:underline">
                      {name}
                    </Link>
                  ) : (
    ...
    <div className="bg-[#0a0a45] py-7 w-full text-center ">
        <p className="text-[#fff]">Copyright © 2024 Myjobs. All Rights Reserved.</p>
    </div>
    </>
  );
}
