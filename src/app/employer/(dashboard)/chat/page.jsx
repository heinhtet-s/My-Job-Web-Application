...
const Page = () => {
  ...
  const fetchEmployerData = async (Id) => {
    try {
      const res = await GetSeekerList(
        `?$filter=Id in (${Id})&$select=FirstName,LastName,Id,ImageUrl`
      );
      console.log(res);
      setSeekerData(res?.value);
    } catch (e) {
      console.log(e);
    }
  };
    ...
    return (
      <div className="flex justify-end mb-4 cursor-pointer">
        ...
        <div className="w-9 h-9 rounded-full flex items-center justify-center ml-2">
          <img
            src={
              seekerData?.find((el) => el?.Id === data?.senderId)?.ImageUrl ||
              "image/no-image.png"
            }
            alt="My Avatar"
            className="w-8 h-8 rounded-full"
          ...
          {chatRooms.map((room) => (
            ...
            >
              <div className="w-12 h-12 bg-gray-300 rounded-full mr-3">
                <img
                  src={
                    seekerData?.find((el) => el?.Id === room?.seekerId)
