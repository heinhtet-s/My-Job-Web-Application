...
const MIN_DIMENSION = 100; // Changed to 100 for 100x100 crop
...
const setCanvasPreview = (image, canvas, crop) => {
  ...
  // Adjust the output width and height, capping them at 100px
  const outputWidth = Math.min(Math.floor(finalWidth * pixelRatio), 100);
  const outputHeight = Math.min(Math.floor(finalHeight * pixelRatio), 100);
...
const EmployerImageCropper = ({ handleSubmit }) => {
          ...
          {crop && (
            <canvas
              ...
              style={{
                ...
                objectFit: "contain",
                width: 100, // Canvas width set to 100
                height: 100, // Canvas height set to 100
              }}
            />
          )}
        </Modal.Body>
...
export default EmployerImageCropper;
